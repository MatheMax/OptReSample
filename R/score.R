...
score <- function( parameters, cf, ce, n1, lambda1, lambda2 ) {
  ...
  for(i in 1:(4*N+1)) {
    ...
    c <- ( a(parameters)^2 * n - b( parameters, x[i], n1, lambda1, lambda2 ) ) / ( 2 * a(parameters) * sqrt(n) )
    y[i] <- n * dnorm( x[i] - a(parameters) * sqrt(n1) )
    y[i] <- y[i] - ( lambda1 * pnorm(c) * dnorm(x[i]) )
    y[i] <- y[i] + ( lambda2 * pnorm( c - a(parameters) * sqrt(n) ) * dnorm( x[i] - a(parameters) * sqrt(n1) ) )
  }
  p <- (2*h)/45*(t(omega)%*%y)
  ...
  g <- g + lambda2 * ( pnorm( cf - a(parameters) * sqrt(n1) ) - parameters$beta )
  g <- g + p
