...
fixed <- function(parameters){
  score_fixed <- function(parameters,n,c,lambda1,lambda2){
    ...
    p <- p + lambda2 * ( - parameters$beta + pnorm( c - sqrt(n) * (parameters$mualt-parameters$mu0) / parameters$sigma ) )
  }
  ...
  optimal <- nloptr::nloptr(
    ...
    eval_g_ineq = function(x) c((1 - pnorm ( x[2] ) - parameters$alpha),
                                - parameters$beta + pnorm( x[2] - sqrt(x[1]) * (parameters$mualt-parameters$mu0) / parameters$sigma )),
    opts = list(
